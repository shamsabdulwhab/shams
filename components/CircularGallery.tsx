...
function createTextTexture(gl: Renderer['gl'], text: string, font: string = 'bold 30px monospace', color: string = '#0575E6') {
...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = '#0575E6', font = 'bold 30px sans-serif' }: {
    gl: Renderer['gl'];
    plane: Mesh;
    renderer: Renderer;
    text: string;
...
class Media {
  ...
  update(scroll: { current: number; last: number; target: number }, direction: string) {
    ...
    const verticalOffset = 2.5; // Move images up by this amount

    if (this.bend === 0) {
      this.plane.position.y = verticalOffset;
...
class App {
  ...
  start: number = 0;
  boundOnResize: () => void = () => {};
  boundOnWheel: (e: WheelEvent | Event) => void = () => {};
  ...
  createMedias(items: Array<{ image: string; text: string }> | undefined, bend: number = 1, textColor: string, borderRadius: number, font: string) {
    const defaultItems = [
      { image: `/src/assets/images/Home page (3).png`, text: 'My Project' },
      { image: ``, text: 'Portfolio Work' },
      { image: ``, text: 'Design Project' },
      { image: ``, text: 'Web Development' },
      { image: ``, text: 'Creative Work' }
    ];
